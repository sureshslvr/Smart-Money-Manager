# 💰 Smart Money Manager - Free AppSheet Template

## Why This App Exists

Most expense tracking solutions focus on subscriptions, premium features, advertisements, or storing your financial data on third-party systems.

This app was built with a different goal:

**A completely free personal finance management solution where your data remains under your control.**

All your financial records are stored in your own Google account and can be accessed, exported, or backed up by you at any time.

The purpose of this app is simple:

> Help individuals and families understand exactly where their money is going and build better saving habits through consistent expense tracking.

Many people know their monthly income but struggle to identify where their money disappears. The only reliable way to improve savings is to track every income and expense, no matter how small.

By recording every transaction, you gain visibility into:

* Spending patterns
* Unnecessary expenses
* Monthly savings potential
* Financial goals progress
* Income vs Expense trends

Even small daily expenses can add up significantly over time. This app helps make those expenses visible.

---

## Key Features

### 📊 Interactive Dashboard

* Monthly income overview
* Expense breakdown
* Savings tracking
* Category-wise analysis
* Visual charts and summaries

### 💵 Income Tracking

* Record salary
* Record side income
* Record bonuses
* Record other earnings

### 🛒 Expense Tracking

* Track daily expenses
* Categorize spending
* Monitor monthly expenses
* Maintain complete spending history

### ♻️ Deleted Entry Recovery

* Deleted entries are not permanently lost
* Restore records from Deleted Expenses view

### 📅 Monthly Analysis

* View dashboard for any month and year
* Compare financial performance across periods
* Analyze spending trends

### ☁️ Your Data, Your Control

* Data remains in your own Google account
* No vendor lock-in
* Easy backup and export options

---

# App Setup Guide

## Step 1: Open the Template

Open the AppSheet template using the link below:

https://www.appsheet.com/Template/AppDef?appName=InventoryManagementTemplate-555308025-26-06-20&utm_source=share_app_link

---

## Step 2: Create Your Own Copy

1. Click **Copy App**
![copy app]([screenshots/click on copy app .jpg](https://github.com/sureshslvr/Smart-Money-Manager/blob/161e03de5b208baa3f43f87762ce629e68f4daf2/screenshots/click%20on%20copy%20app%20.jpg))
2. Select **Copy App Details**
3. Click **Copy App**
4. Wait for AppSheet to create your personal copy

---

## Step 3: Deploy the App

After the copy is completed:

1. Open your copied app
2. Click **Deploy App** from the top-left corner
3. Complete any deployment checks shown by AppSheet

---

## Step 4: Install the AppSheet Mobile App

Download the AppSheet mobile application:

Android:
https://play.google.com/store/apps/details?id=com.google.appsheet.whitelabel.guidde

---

## Step 5: Login

Login to the AppSheet mobile application using the same Google account used to deploy the app.

After login, the application will automatically appear in your account.

---

# First Time Configuration

## Update Monthly Salary

To start using the app:

1. Open the **Salary** section
2. Locate the sample **February Salary** record
3. Open the record
4. Click the **Three Dots Menu**
5. Select **Edit**
6. Update:

   * Date
   * Salary Amount
   * Income Details
7. Click **Save**

Refer to the screenshots provided in this repository.

---

# Adding Income

To add a new income entry:

1. Open the Income section
2. Click the **+ (Add)** button
3. Enter income details
4. Click **Save**

Examples:

* Salary
* Bonus
* Freelancing
* Business Income
* Other Earnings

---

# Adding Expenses

To add a new expense:

1. Open **My Expenses**
2. Click the **+ (Add)** button
3. Enter expense details
4. Select category
5. Click **Save**

For best results, record expenses immediately after spending.

Examples:

* Groceries
* Fuel
* Rent
* Shopping
* Utilities
* Entertainment

---

# Deleting Entries

To delete an entry:

1. Open the entry
2. Click Delete

or

1. Select one or more entries
2. Click Delete

---

# Restoring Deleted Entries

Deleted entries can be restored.

Steps:

1. Open the navigation menu (☰)
2. Select **Deleted Expenses**
3. Select the record(s) to restore
4. Click **Undo**

The entry will be restored to active records.

---

# Viewing Monthly Dashboard

To view financial insights for any month:

1. Open **Dashboard**
2. Edit the Month field
3. Edit the Year field
4. Save the selection

The dashboard will automatically refresh and display:

* Total Income
* Total Expenses
* Savings
* Category Analysis
* Monthly Trends

---

# Recommended Usage

For accurate financial insights:

✅ Record every income

✅ Record every expense

✅ Update transactions daily

✅ Review dashboard weekly

✅ Analyze spending patterns monthly

The more accurately you record transactions, the more valuable the dashboard becomes.

---

# Screenshots

Refer to the screenshots folder for:

* App Setup
* Editing Salary
* Adding Expenses
* Adding Income
* Restoring Deleted Entries
* Dashboard Overview

---

# Support

If you find any issues or have suggestions for improvements, feel free to create an issue in this repository.

Contributions and feedback are always welcome.

